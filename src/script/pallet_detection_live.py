...
class PalletDetectionLive:
    ...
    def run(self):
        ...
        requests = [
        build_image_request(self.options.image_source, quality_percent=self.options.jpeg_quality_percent,
                            resize_ratio=self.options.resize_ratio)]
        
        print("\nCorner detection is starting, this might take a while...\n")
        print("Camera: {}".format(self.options.image_source))
        ...
        while True:
            ...
            for i in range(len(images)):
                processed_image, center_x, center_y = self.process_image(images[i])
